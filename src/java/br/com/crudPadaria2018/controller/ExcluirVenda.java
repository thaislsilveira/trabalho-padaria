 ...
 */
@WebServlet(name = "ExcluirVenda", urlPatterns = {"/ExcluirVenda"})
public class ExcluirVenda extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       try{
           ...
           if(dao.excluir(idVenda)){
               mensagem = "Venda excluido com sucesso!";
           } else {
               mensagem = "Problemas ao excluir Venda!";
           }
           request.setAttribute("mensagem", mensagem);
           request.getRequestDispatcher("ListarVenda").forward(request, response);
       }catch(Exception ex){
           System.out.println("Problemas no Servlet ao excluir Venda! Erro: " + ex.getMessage());
       }
    }
