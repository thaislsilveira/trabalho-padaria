 ...
 * @author Servidor
 */
@WebServlet(name = "AlterarVendedor", urlPatterns = {"/AlterarVendedor"})
public class AlterarVendedor extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       oVendedor.setIdVendedor(idVendedor);
       oVendedor.setNomeVendedor(nomeVendedor);
     
              
       try{
           GenericDAO dao = new VendedorDAO();
           if(dao.cadastrar(oVendedor)){
               mensagem = "Vendedor cadastrado com sucesso!";
            }else{
               mensagem = "Problemas ao cadastrar Vendedor. Verifique os dados informados e "
                       + "tente novamente";
           }
           request.setAttribute("mensagem", mensagem);
           request.getRequestDispatcher("ListarVendedor").forward(request, response);
       }catch(Exception ex){
